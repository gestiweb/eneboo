...
public:
   ...
   * False otherwise.
   */
  bool operator==(const QSListIterator &it) const {
    return node == it.node;
  }
  /**
...
inline void QSList::append(const QSObject &obj)
{
...
inline void QSList::prepend(const QSObject &obj)
{
...
inline void QSList::removeFirst()
{
  erase(hook->next);
}
inline void QSList::removeLast()
{
  erase(hook->prev);
}
inline void QSList::remove(const QSObject &obj)
{
  Q_UNUSED(obj)
...
inline QSList *QSList::copy() const
{
...
inline void QSList::erase(ListNode *n)
{
  if (n != hook) {
    n->next->prev = n->prev;
    n->prev->next = n->next;
...
inline QSObject QSList::at(int i) const
{
  const ListNode *n = hook->next;
  while (i > 0 && n != hook) {
    n = n->next;
    --i;
  }
