...
IdeWindow *get_workbench(QWidget *w)
{
  if (!w)
    return 0;
  IdeWindow *wb = ::qt_cast<IdeWindow *>(w->topLevelWidget());
  return (wb ? wb : get_workbench(w->parentWidget()));
}
#endif
...
public:
  QSProjectPrivate() :
    ...
    needsRerun(FALSE),
    bundleStorage(FALSE) {
    dictScripts.resize(101);
    dictObjects.resize(101);
  }
  ...
  // ### AbanQ
  QDict<QSScript> dictScripts;
  QDict<QObject> dictObjects;
};
...
{
  QSScript *s = d->dictScripts.find(name);
  if (s)
    return s;
  QPtrListIterator<QSScript> it(d->scripts);
  while ((s = it())) {
    if (s->QSScript::name() == name) {
      d->dictScripts.insert(name, s);
      return s;
    }
  }
  return 0;
...
QSScript *QSProject::createScript(QObject *context, const QString &code)
{
  ...
  if (strcmp(context->name(), "unnamed") == 0)
    qWarning("QSProject::createScript: unnamed object");
#endif
...
void QSProject::addObject(QObject *object)
{
  ...
  QString oName(object->name());
  d->objects.append(object);
  d->dictObjects.insert(oName, object);
  // ### AbanQ
  QSScript *s = script(oName);
  if (s)
    s->setContext(object);

  getQuickInterpreter()->addTopLevelObject(object);
  if (!d->needsRerun) // ### AbanQ
    emit projectChanged();
}
...
void QSProject::removeObject(const QObject *object)
{
  ...
  disconnect(object, SIGNAL(destroyed()),
             this, SLOT(objectDestroyed()));
  QString oName(object->name());
  d->objects.removeRef(object);
  d->dictObjects.remove(oName);
  QSScript *s = script(oName);
  if (s) {
    d->scripts.removeRef(s);
    d->dictScripts.remove(s->QSScript::name());
  }
                                          ...
                                          QObject *context)
{
  ...
  if (context &&  d->objects.findRef(context) < 0) {
...
void QSProject::objectDestroyed()
{
  ...
  if (d->editors.removeRef((QSEditor *) o))
    return;
  QSScript *s = (QSScript *) o;
  if (d->scripts.removeRef(s)) {
    d->dictScripts.remove(o->name());
    if (!d->needsRerun) // ### AbanQ
      emit projectChanged();
    return;
  }
...
//### AbanQ
#ifdef QSDEBUGGER
QuickDebugger *QSProject::debugger() const
{
  return getQuickInterpreter()->debuggerEngine();
}
#endif
