...
namespace MIMECodec
{
    ...
    AQMAIL_EXPORT QCString canonicalForm(const QCString &);

    /** translate a MIME-encoded text.*/
    AQMAIL_EXPORT QString translate(const QCString &);

    /** MIME-fold text (used for headers).*/
    AQMAIL_EXPORT QCString fold(const QCString &);

    /** unfold MIME-folded text.*/
    AQMAIL_EXPORT QCString unfold(const QCString &);

    /** encode UNIX text (cr-ended) into MIME text (crlf-ended).*/
    AQMAIL_EXPORT QCString encodeEndLine(const QCString &);

    /** decode MIME text (crlf-ended) into UNIX text (cr-ended).*/
    AQMAIL_EXPORT QCString decodeEndLine(const QCString &);

    /** encode text in quoted-printable format (the bool parameter forces extra MIME gateway compatibility).*/
    AQMAIL_EXPORT QCString encodeQuotedPrintable(const QCString &, bool =false, bool =false);

    /** decode quoted-printable encoded text.*/
    AQMAIL_EXPORT QCString decodeQuotedPrintable(const QCString &);

    /** encode binary data in base64 format.*/
    AQMAIL_EXPORT QByteArray encodeBase64(const QByteArray &, bool endNewLine=true, bool endAllNewLines=true);

    /** decode base64-encoded binary data.*/
    AQMAIL_EXPORT QByteArray decodeBase64(const QByteArray &);
    ...
    AQMAIL_EXPORT QByteArray encode(const QByteArray &, const char *, bool =false);   // buffer, encoding

    /** decode binary data using encoding name.*/
    AQMAIL_EXPORT QByteArray decode(const QByteArray &, const char *); // encoded buffer, encoding

    /** encode the message id.
      * It has a unique number/string and the user name like parameter.
      */
    AQMAIL_EXPORT QString encodeMessageId(const QString &, const QString &);

    /** create a cram-md5-digest using username, password, challenge
      * from Rik Hemsley (rikkus) <rik@kde.org>
      */
    AQMAIL_EXPORT QByteArray CramMd5Digest(const QCString&,const QCString&,const QCString&);
};
