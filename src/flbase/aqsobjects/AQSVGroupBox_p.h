...
#ifndef AQSVGROUPBOX_P_H_
#define AQSVGROUPBOX_P_H_

#include "AQSGroupBox_p.h"
#include <qvgroupbox.h>

class AQSVGroupBox : public AQSGroupBox
{
  Q_OBJECT

  AQ_DECLARE_AQS_OBJECT(VGroupBox, GroupBox);
...
protected:
  static void *construct(const QSArgumentList &args) {
    ...
    if (sgt.isEmpty())
      return new QVGroupBox;
    if (sgt == QString::fromLatin1("QWidget*"))
      return new QVGroupBox(argValue<QWidget *>(args[0]));
    if (sgt == QString::fromLatin1("QWidget*,QString"))
      return new QVGroupBox(argValue<QWidget *>(args[0]),
                            *(argValue<QString *>(args[1])));
    if (sgt == QString::fromLatin1("QString"))
      return new QVGroupBox(*(argValue<QString *>(args[0])));
