...
#ifndef AQSHBUTTONGROUP_P_H_
#define AQSHBUTTONGROUP_P_H_

#include "AQSButtonGroup_p.h"
#include <qhbuttongroup.h>

class AQSHButtonGroup : public AQSButtonGroup
{
  Q_OBJECT

  AQ_DECLARE_AQS_OBJECT(HButtonGroup, ButtonGroup);
...
protected:
  static void *construct(const QSArgumentList &args) {
    ...
    if (sgt.isEmpty())
      return new QHButtonGroup;
    if (sgt == QString::fromLatin1("QWidget*"))
      return new QHButtonGroup(argValue<QWidget *>(args[0]));
    if (sgt == QString::fromLatin1("QWidget*,QString"))
      return new QHButtonGroup(argValue<QWidget *>(args[0]),
                               *(argValue<QString *>(args[1])));
    if (sgt == QString::fromLatin1("QString"))
      return new QHButtonGroup(*(argValue<QString *>(args[0])));
