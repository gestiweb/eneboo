...
#ifndef AQSRADIOBUTTON_P_H_
#define AQSRADIOBUTTON_P_H_

#include "AQSButton_p.h"
#include <qradiobutton.h>

class AQSRadioButton : public AQSButton
{
  Q_OBJECT

  AQ_DECLARE_AQS_OBJECT(RadioButton, Button);
...
protected:
  static void *construct(const QSArgumentList &args) {
    ...
    if (sgt == QString::fromLatin1("QWidget*"))
      return new QRadioButton(argValue<QWidget *>(args[0]));
    if (sgt == QString::fromLatin1("QWidget*,QString"))
      return new QRadioButton(argValue<QWidget *>(args[0]),
                              *(argValue<QString *>(args[1])));
    if (sgt == QString::fromLatin1("QString,QWidget*"))
      return new QRadioButton(*(argValue<QString *>(args[0])),
