...
public:
  ...
  Q3SqlCursorPrivate( const QString& name, QSqlDatabase sdb )
      : lastAt( QSql::BeforeFirstRow ), nm( name ), srt( name ), md( 0 ), db( sdb ), q( 0 )
  {}
  ~Q3SqlCursorPrivate()
  {
    delete q;
...
QString qOrderByClause( const QSqlIndex & i, const QString& prefix = QString() )
{
  ...
  if ( k == 0 )
    return QString();
  str = QLatin1String( " order by " ) + sqlIndexToString( i, prefix );
  return str;
}
...
bool Q3SqlCursor::select( const QString & filter, const QSqlIndex & sort )
{
  ...
  if ( !filter.isEmpty() ) {
    ...
    d->ftr = QString();
  if ( sort.count() > 0 )
    str += QLatin1String( " order by " ) + sqlIndexToString( sort, d->nm );
  d->srt = sort;
  return exec( str );
}
