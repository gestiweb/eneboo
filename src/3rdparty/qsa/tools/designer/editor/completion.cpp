...
{
  if ( type == QLatin1String("function") || type == QLatin1String("slot") || type == QLatin1String("package") )
    return Qt::blue;
  else if ( type == QLatin1String("variable") || type == QLatin1String("widget") || type == QLatin1String("dir") )
    return Qt::darkRed;
  else if ( type == QLatin1String("object") || type == QLatin1String("class") )
    return Qt::darkBlue;
...
public:
  ...
  QString text() const
  {
    return Q3ListBoxItem::text() + postfix;
  }
...
void CompletionItem::setupParagraph()
{
  if ( !parag ) {
    ...
    parag->setTabStops( listBox()->fontMetrics().width( QLatin1String("propertyXXXX") ) );
...
bool EditorCompletion::eventFilter( QObject *o, QEvent *e )
{
  ...
  if ( e->type() == QEvent::KeyPress &&
    ...
    if ( ke->key() == Qt::Key_Tab ) {
      ...
      if ( curEditor->document()->hasSelection( Q3TextDocument::Standard ) ||
        ...
        if ( curEditor->document()->indent() ) {
          ...
          curEditor->insert( QLatin1String("\t") );
        }
        return true;
      }
    }
    ...
    if ( (ke->text().length() && !( ke->state() & Qt::AltModifier ) &&
         ...
         ( ke->text() == QLatin1String("\t") && !( ke->state() & Qt::ControlModifier ) ) ) {
      if ( ke->key() == Qt::Key_Tab ) {
        ...
        if ( !doArgumentHint( ke->text() == QLatin1String("(") ) )
          functionLabel->hide();
      }
    }
