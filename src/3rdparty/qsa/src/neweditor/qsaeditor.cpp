...
QuickInterpreter *QSAEditor::interpreter() const
{
  ...
  if ( !qsInterp )
    printf( "QSAEditor::interpreter(), using defaultInterpreter\n" );
#endif
...
QSObject QSAEditor::queryQSObject( QSObject &ctx, const QString &property ) const
{
  ...
  if ( s.indexOf( '[' ) != -1 )
    ...
    return env() ->createString( NEW_USTR( "" ) );
  return ctxValue;
}
...
QSObject QSAEditor::vTypeToQSType( const QString &type ) const
{
  ...
  case QVariant::DateTime:
    return env() ->globalObject().get( NEW_USTR( "Date" ) );
  case QVariant::StringList:
  case QVariant::List:
  case QVariant::Map:
    return env() ->globalObject().get( NEW_USTR( "Array" ) );
  case QVariant::ByteArray:
    return env() ->globalObject().get( NEW_USTR( "String" ) );
  case QVariant::Int:
  case QVariant::UInt:
  case QVariant::Double:
    return env() ->globalObject().get( NEW_USTR( "Number" ) );
  case QVariant::Bool:
    return env() ->globalObject().get( NEW_USTR( "Boolean" ) );
  default:
    break;
  }
...
QString QSAEditor::cppClassForScript( const QString &className ) const
{
  ...
  QMap<uint, QString>::ConstIterator cppName = names.find( NEW_USTR( className ) );
...
void QSAEditor::doObjectCompletion()
{
  ...
  if ( objectName == object ) {
    QSObject stobj = env() ->globalObject().get( NEW_USTR( object ) );
