...
void QSClass::clear()
{
    for ( QSMemberMap::ConstIterator it = mmap->begin(); it != mmap->end(); ++it ) {
        if ( ( *it ).type() == QSMember::ScriptFunction ) {
            if ( ( *it ).scriptFunction && ( *it ).scriptFunction->deref() ) {
...
*/
#if defined(QSDEBUGGER) && !defined(QT_NO_DEBUG)
QSEqualsResult QSClass::isEqual( const QSObject &a, const QSObject & /*b*/ ) const
{
    Q_ASSERT( a.isA( this ) );
    //qDebug( "QSClass::isEqual( %s, %s )", a.typeName().ascii(), b.typeName().ascii() );
#else
QSEqualsResult QSClass::isEqual( const QSObject &, const QSObject & ) const
{
#endif
...
#endif
    for ( QSMemberMap::ConstIterator it = mmap->begin(); it != mmap->end(); ++it )
        if ( ( *it ).type() == QSMember::ScriptFunction && ( *it ).scriptFunction ) {
...
const QSClassClass* QSClass::asClass() const
{
    return name() == QLatin1String( "Class" ) ? static_cast<const QSClassClass *>( this ) : 0;
}
...
{
    return QLatin1String( "[object " ) + name() + QLatin1String( "]" );
}

uint QSClass::toUString( const QSObject * o ) const
{
    return NEW_USTR( this->toString( o ) );
}
...
bool QSClass::member( const QSObject *, uint n, QSMember *m ) const
{
    Q_ASSERT( m );
    Q_ASSERT( mmap );
                              ...
                              const QSMember &mem ) const
{
    ...
    if ( mem.type() == QSMember::Variable ) {
        if ( !mem.isStatic() ) {
            QSInstanceData * data = static_cast<QSInstanceData *>( objPtr->shVal() );
                     ...
                     const QSObject &val ) const
{
    ...
    if ( mem.type() != QSMember::Variable ) {
        env() ->throwError( ReferenceError,
                            QString( "Member '%1' cannot be overwritten in '%2'" )
    ...
    if ( mem.isWritable() && mem.type() == QSMember::Variable ) {
        if ( !mem.isStatic() ) {
            ...
            QSClass * cl = const_cast<QSClass *>( this );
...
{
    QSInstanceData * idata = static_cast<QSInstanceData *>( objPtr->shVal() );
...
QSObject QSClass::invoke( QSObject * objPtr, const QSMember &mem ) const
{
    ...
    case QSMember::ScriptFunction: {
            ...
            QString argStr = QLatin1String( "" );
            for ( int j = 0; j < args->size(); j++ ) {
                if ( j > 0 )
                    argStr += QLatin1String( ", " );
                QSObject a = args->at( j );
                argStr += a.toString() + QLatin1String( " : " ) + a.typeName();
            }
            QString n = USTR( mem.scriptFunction->scopeDefinition() ->identifier() );
            if ( dbg )
...
void QSClass::addFunctionMember( uint n, QSFunctionBodyNode * f, int attributes )
{
    addMember( n, QSMember( f, attributes ), createUndefined() );
}

int QSClass::addVariableMember( uint n, int attributes )
{
...
void QSClass::addStaticVariableMember( uint name, const QSObject &value, int attr )
{
 ...
 */
void QSClass::addMember( uint n, const QSMember &member, const QSObject &stVal )
{
...
*/
void QSClass::replaceMember( uint name, QSMember *member,
                             const QSObject &stVal )
{
    ...
    if ( old.type() == QSMember::ScriptFunction && old.scriptFunction ) {
    ...
    // Ref new one...
    if ( m.type() == QSMember::ScriptFunction && m.scriptFunction )
...
/*!
  ...
  is returned
*/
bool QSClass::deleteMember( uint name )
{
...
QSObject QSClass::get( const QSObject *objPtr, uint p ) const
{
    QSMember mem;
...
void QSClass::put( QSObject *objPtr, uint p, const QSObject &v ) const
{
    QSMember mem;
    if ( !member( objPtr, p, &mem ) && mem.type() != QSMember::Identifier )
        return ;
...
{
    return en->createString( NEW_USTR( s ) );
}
...
uint QSUndefinedClass::toUString( const QSObject * ) const
{
    return QSLexer::undefined_;
}
...
QSEqualsResult QSUndefinedClass::isEqual( const QSObject &a, const QSObject &b ) const
{
	Q_ASSERT( a.isA( this ) );
#else
QSEqualsResult QSUndefinedClass::isEqual( const QSObject &, const QSObject &b ) const
{
#endif
...
uint QSNullClass::toUString( const QSObject * ) const
{
    return QSLexer::null_;
}
...
QSEqualsResult QSNullClass::isEqual( const QSObject &a, const QSObject &b ) const
{
	Q_ASSERT( a.isA( this ) );
#else
QSEqualsResult QSNullClass::isEqual( const QSObject &, const QSObject &b ) const
{
#endif
...
uint QSCharacterClass::toUString( const QSObject *obj ) const
{
    return obj->sVal();
}
...
void QSSharedClass::deref( QSObject * o ) const
{
    ...
    if ( o->shVal() ->count == 0 ) {
        ...
        delete o->shVal();
        o->setVal( static_cast<QSShared *>( 0 ) ); // for debugging purposes only
    }
#endif
}

QSClassClass::QSClassClass( QSClass *b, int a, uint n )
        : QSSharedClass( b, a ), cname( n ), defaultCtor( false ), bodyNode( 0 ), clDefNode( 0 )
{
    sname = QLatin1String( "[class " ) + USTR( cname ) + QLatin1String( "]" );
    uname = NEW_USTR( sname ); REF_USTR( uname );
...
uint QSClassClass::toUString( const QSObject * ) const
{
    return uname;
}
...
{
    return static_cast<QSInstanceData *>( obj->shVal() );
}

const QSInstanceData* QSClassClass::data( const QSObject *obj )
{
    return static_cast<const QSInstanceData *>( obj->shVal() );
}
...
QSObject QSClassClass::construct( const QSList &args ) const
{
    ...
    if ( baseChain && baseChain->name() == QLatin1String( "AbstractBase" ) ) {
        return env() ->throwError( QString( QString( "class '%1' is %2derived from undefined class '%3'" ) )
                                   .arg( cname )
                                   .arg( baseChain == base()
                                         ? QLatin1String( "" )
                                         : QLatin1String( "indirectly " ) )
...
{
    int offset = 0;
    const QSClassClass *cl = base() ? base() ->asClass() : 0;
    if ( cl )
...
bool QSWritableClass::member( const QSObject *o, uint n,
                              QSMember *m ) const
{
    Q_ASSERT( m );
    ...
    const QSWritable *w = static_cast<QSWritable *>( o->shVal() ); //data( o );
...
{
    return static_cast<QSWritable *>( obj->shVal() );
}

const QSWritable *QSWritableClass::data( const QSObject *obj )
{
    return static_cast<const QSWritable *>( obj->shVal() );
}
...
QSFunctionScopeClass::QSFunctionScopeClass( QSClass *b, QSFuncDeclNode * func )
        : QSWritableClass( b ), ident( func->identifier() ),
        numArgs( 0 ), body_node( 0 )
{}

uint QSFunctionScopeClass::identifier() const
{
    return !ident ? QSLexer::anony_ : ident;
}
...
QSObject QSFunctionScopeClass::construct( const QSList &args ) const
{
    ...
    for ( i = 0; it != args.end(); ++i, ++it )
        argObj.put( NEW_USTRNUM( i ), *it );
    scope.put( QSLexer::args_, argObj );
...
QSClass *QSTypeClass::classValue( const QSObject *obj )
{
    ...
    return static_cast<QSTypeClassShared *>( obj->shVal() ) ->classValue;
}

bool QSTypeClass::member( const QSObject *o, uint n,
                          QSMember *m ) const
{
    if ( !o )
        return false;
...
QSMemberMap QSTypeClass::members( const QSObject *obj ) const
{
    ...
    if ( static_cast<const QSClass *>( classValue( obj ) ) == this )
                                  ...
                                  const QSMember &mem ) const
{
    ...
    if ( !mem.hasAttribute( AttributeStatic ) ) {
        throwError( ReferenceError, QLatin1String( "Cannot access a non-static member "
...
QSObject QSTypeClass::invoke( QSObject *o, const QSMember &mem ) const
{
    ...
    throwError( ReferenceError, QLatin1String( "Cannot invoke a non-static function "
                "without an object reference" ) );
    return createUndefined();
}
                         ...
                         const QSObject &val ) const
{
    ...
    if ( mem.type() == QSMember::Variable ) {
        ...
        throwError( ReferenceError, QLatin1String( "Trying to write to a nonvariable" ) );
        return ;
    }
}
...
{
    printf( "class %s", USTR( cl->identifier() ).toLatin1().constData() );
    ...
    for ( ; it != cl->definedMembers() ->end(); ++it ) {
        QSMember mem = *it;
        QString line = QLatin1String( "  " ) + mem;
...
static void qs_dumpobject( const QSObject &obj )
{
    ...
    printf( "class %s :: %s\n",
            ...
            USTR( cl->identifier() ).toLatin1().constData() );
    QSMemberMap::Iterator it = cl->definedMembers() ->begin();
    for ( ; it != cl->definedMembers() ->end(); ++it ) {
        QSMember mem = *it;
        if ( mem.isReadable() ) {
            ...
            if ( mem.type() == QSMember::Variable )
                printf( "  %2d: %s = %s\n", mem.index(), USTR( mem.name() ).toLatin1().constData(), value.toString().toLatin1().constData() );
            else
                printf( "      %s = %s\n", USTR( mem.name() ).toLatin1().constData(), value.toString().toLatin1().constData() );
        }
    }
}
...
{
    addMember( REG_USTR( QLatin1String( "dumpObject" ) ),
               QSMember( &dumpObject, AttributeNonWritable | AttributeStatic ) );
    addMember( REG_USTR( QLatin1String( "dumpScope" ) ),
               QSMember( &dumpScope, AttributeNonWritable | AttributeStatic ) );
    addMember( REG_USTR( QLatin1String( "dumpType" ) ),
...
QSSystemClass::QSSystemClass( QSClass *base )
        : QSClass( base, AttributeAbstract )
{
    addMember( REG_USTR( QLatin1String( "print" ) ),
               QSMember( &print, AttributeNonWritable | AttributeStatic ) );
    addMember( REG_USTR( QLatin1String( "println" ) ),
               QSMember( &println, AttributeNonWritable | AttributeStatic ) );
    addMember( REG_USTR( QLatin1String( "getenv" ) ),
               QSMember( &getenv, AttributeNonWritable | AttributeStatic ) );
    addMember( REG_USTR( QLatin1String( "setenv" ) ),
...
{
    return env->createString( NEW_USTR( QLatin1String( ::getenv( env->arg( 0 ).toString().toLatin1().constData() ) ) ) );
}
...
QVariant QSDynamicClass::toVariant( const QSObject *obj, QVariant::Type ) const
{
...
void QSAbstractBaseClass::replace( QSClassClass *newBase )
{
    ...
    QSClassList allClasses = env() ->classes();

    // Build a list of the user classes, excluding this one.
    for ( QSClassList::const_iterator it = allClasses.begin(); it != allClasses.end(); ++it ) {
        if ( ( *it ) ->asClass() && ( *it ) != newBase )
            userClasses.append( static_cast<QSClassClass *>( *it ) );
    }
...
QString operator+( const QString &a, const QSMember &b )
{
    ...
    s.sprintf( "QSMember(%s.%s, %s, %x)",
               b.owner() ? USTR( b.owner() ->identifier() ).toLatin1().constData() : "(no owner)",
               USTR( b.name() ).toLatin1().constData(),
