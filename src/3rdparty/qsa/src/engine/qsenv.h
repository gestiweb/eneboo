...
public:
    ...
    QSClass* classByIdentifier(uint n);

    void pushLabel(uint l);
    void popLabel();
    bool containsLabel(uint l) const;
    uint currentLabel() const { return label; }
    void setCurrentLabel(uint l) { label = l; }
    ...
    QSObject resolveValue(uint n) const;

    QSClassList classes() const { return classList; }
    ...
    QSObject createString(uint s) const;
...
inline QSObject QSEnv::createString(uint s) const
{
