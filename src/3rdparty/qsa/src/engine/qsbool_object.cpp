...
{
    return env->createString( NEW_USTR( env->thisValue().toString() ) );
}
...
{
    addMember( REG_USTR( QLatin1String( "toString" ) ), QSMember( bToString ) );
    addMember( REG_USTR( QLatin1String( "valueOf" ) ), QSMember( bValueOf ) );
}
...
{
    return obj->bVal() ? QLatin1String( "true" ) : QLatin1String( "false" );
}

uint QSBooleanClass::toUString( const QSObject *obj ) const
{
    return obj->bVal() ? QSLexer::true_ : QSLexer::false_;
}
