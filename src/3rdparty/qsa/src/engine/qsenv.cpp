...
void QSEnv::clear()
{
    ...
    for ( int i = 0; i < classList.size(); ++i )
        classList.at( i ) ->clear();

#ifndef QS_LEAK
    while ( !classList.isEmpty() )
        delete classList.takeFirst();
#endif
...
QSClass* QSEnv::classByName( const QString &n )
{
    ...
    if ( n == QLatin1String( "Null" ) )
        t = nullClass();
    else if ( n == QLatin1String( "Number" ) )
        t = numberClass();
    else if ( n == QLatin1String( "Boolean" ) )
        t = booleanClass();
    else if ( n == QLatin1String( "String" ) )
        t = stringClass();
    else
...
QSClass* QSEnv::classByIdentifier( uint n )
{
    for ( int i = 0; i < classList.size(); ++i )
...
void QSEnv::printScopeChain() const
{
    ...
    for ( int i = 0; i < scopeChain->size(); ++i ) {
        ...
        QString tmp = !obj.isValid() ? QLatin1String( "#" )
                      : USTR( obj.objectType() ->identifier() );
        lst.append( tmp );
    }
    qDebug( "Current scope is:: %s", lst.join( QLatin1String( ", " ) ).toLatin1().constData() );
}
...
QSObject QSEnv::resolveValue( uint ident ) const
{
    Q_ASSERT( ident > 0 );
...
void QSEnv::popLabel()
{
    ...
    Q_ASSERT( labels.front() > 0 );
    labels.pop_front();
}

bool QSEnv::containsLabel( uint label ) const
{
    QList<uint>::const_iterator it = labels.begin();
    while ( it != labels.end() && ( *it ) > 0 ) {
        if ( *it == label )
            return true;
        ++it;
    }
...
QSObject QSEnv::getValueDirect( int index, int level )
{
    ...
    return *( static_cast<QSInstanceData *>( sclvl.shVal() ) ->value( index ) );
}

void QSEnv::setValueDirect( int index, int level, const QSObject &val )
{
    QSObject & sclvl = ( *scopeChain ) [ level ];
    static_cast<QSInstanceData *>( sclvl.shVal() ) ->setValue( index, val );
}
