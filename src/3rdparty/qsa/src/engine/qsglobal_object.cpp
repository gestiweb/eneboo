...
static void postfixAdd( const QSClass *cl, QSCheckData *data )
{
    ...
    data->enterClass( const_cast<QSClass *>( cl ) );
}
...
static QSObject qsEval( QSEnv *env )
{
    ...
    if ( !x.isString() ) {
        ...
        QSObject res = static_cast<QSProgramNode *>( progNode ) ->execute( env );
...
{
    if ( radix <= 10 ) {
        return QRegExp( QString::fromLatin1( "\\w*(-?[0-%1]+)" ).arg( radix - 1 ) );
    } else if ( radix == 16 ) {
        return QRegExp( QString::fromLatin1( "\\w*(-?[0-9a-fA-Fx]+)" ) );
    } else {
        return QRegExp( QString::fromLatin1( "\\w*(-?[0-9a-%1A-%2]+)" ).arg( 'a' + ( radix - 9 ) ).arg( 'A' + ( radix - 9 ) ) );
    }
}
...
static QSObject qsParseInt( QSEnv *env )
{
    ...
    if ( env->numArgs() == 1 ) {
        if ( str.startsWith( QLatin1String( "0x" ) ) )
            radix = 16;
        else if ( str.startsWith( QLatin1String( "0" ) ) )
            radix = 8;
        else
            radix = 10;
    } else {
...
{
    QRegExp re( QLatin1String( "[+-]?(\\d*)(\\.(\\d*))?([Ee]([+-]?\\d+))?" ) );
    Q_ASSERT( re.isValid() );
...
static QSObject qsEscape( QSEnv *env )
{
    const char non_escape[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                              ...
                              "0123456789@*_+-./";
    QString r = QLatin1String( "" ), str = env->arg( 0 ).toString();
...
{
    // ### DontEnum
    addMember( REG_USTR( QLatin1String( "eval" ) ), QSMember( &qsEval ) );
    addMember( REG_USTR( QLatin1String( "parseInt" ) ), QSMember( &qsParseInt ) );
    addMember( REG_USTR( QLatin1String( "parseFloat" ) ), QSMember( &qsParseFloat ) );
    addMember( REG_USTR( QLatin1String( "isNaN" ) ), QSMember( &qsIsNaN ) );
    addMember( REG_USTR( QLatin1String( "isFinite" ) ), QSMember( &qsIsFinite ) );
    addMember( REG_USTR( QLatin1String( "escape" ) ), QSMember( &qsEscape ) );
    addMember( REG_USTR( QLatin1String( "unescape" ) ), QSMember( &qsUnescape ) );

    addStaticVariableMember( REG_USTR( QLatin1String( "NaN" ) ), createNumber( NaN ), AttributePublic );
    addStaticVariableMember( REG_USTR( QLatin1String( "Infinity" ) ), createNumber( Inf ), AttributePublic );
    addStaticVariableMember( REG_USTR( QLatin1String( "undefined" ) ), createUndefined(), AttributePublic );
}
