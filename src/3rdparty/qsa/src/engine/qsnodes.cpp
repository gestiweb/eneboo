...
#define updateErrorline() \
    if( env->isExceptionMode() ) { \
      ...
      if( QSErrorClass::errorLine(&err) == - 1 ) \
 ...
 */
 ...
 #define ref_printf( a, b, c, d ) printf( a, b, c, d );
#else
...
{
  return env->throwError( e, QLatin1String( msg ), lineNo() );
}

#ifdef QSDEBUGGER
void QSStatementNode::setLoc( int line0, int line1, QSLexer * l )
{
  l0 = line0;
  l1 = line1;
  sid = l->sourceId();
...
QSObject QSResolveNode::rhs( QSEnv * env ) const
{
  ...
  if ( o.isValid() )
    ...
    if ( dbg )
      dbg->setSourceId( env->lexer() ->sourceId() );
#endif

    return env->throwError( QString( "Use of right hand undefined variable '%1'" ).arg( USTR( ident ) ) );
  }
}
...
QSReference QSResolveNode::lhs( QSEnv * env )
{
  ...
  QString msg = QString( "Use of left hand undefined variable %1" ).arg( USTR( ident ) );
...
QSObject QSArrayNode::rhs( QSEnv *env ) const
{
  ...
  if ( element ) {
    ...
    length = opt ? array.get( QSLexer::length_ ).toInt32() : 0;
  } else {
    array = QSArray( env );
    length = 0;
  }
  ...
  if ( opt )
    array.put( QSLexer::length_, env->createNumber( elisionLen + length ) );
...
QSObject QSElementNode::rhs( QSEnv *env ) const
{
  ...
  if ( list ) {
    array = list->rhs( env );
    val = node->rhs( env );
    length = array.get( QSLexer::length_ ).toInt32();
  } else {
    array = QSArray( env );
    val = node->rhs( env );
  }

  array.put( NEW_USTRNUM( elisionLen + length ), val );
...
QSObject QSPropertyNode::rhs( QSEnv *env ) const
{
  QSObject s;
  if ( !str )
    s = env->createString( NEW_USTR( QSString::from( numeric ) ) );
  else
...
QSReference QSAccessorNode1::lhs( QSEnv *env )
{
  ...
  QSMember mem;
  int offset = 0;
  uint id = v2.toUString();
  const QSClass *cl = v1.resolveMember( id, &mem, v1.objectType(), &offset );
  Q_ASSERT( !offset );
  if ( !mem.isDefined() ) {
...
QSObject QSAccessorNode1::rhs( QSEnv *env ) const
{
  ...
  QSMember mem;
  int offset = 0;
  const QSClass *cl = v1.resolveMember( v2.toUString(), &mem, v1.objectType(), &offset );
  Q_ASSERT( !offset );
  if ( cl && mem.isDefined() ) {
    ...
    if ( obj.isUndefined() && mem.type() == QSMember::Identifier ) {
      ...
      return env->throwError( QString( "Trying to access undefined member '%1'" ).arg( s ) );
    }
    updateErrorline();
    return obj;
  }
...
{
  QString ustr( USTR( ident ) );
  bool b = false;
  QString baseIdent( ustr );
  if ( ustr.startsWith( "__" ) ) {
    baseIdent = ustr.right( ustr.length() - 2 );
    b = true;
  }
  ...
  QSMember mem;
  int offset = 0;
  uint uBaseIdent = HASH_USTR( baseIdent );
  const QSClass *cl = v.resolveMember( uBaseIdent, &mem, v.objectType(), &offset );
  ...
  if ( b && cl && mem.isDefined() ) {
    ...
    if ( isFunc ) {
      QString funName = USTR( ( *it ).objectType() ->identifier() );
      QString className = funName.left( funName.indexOf( '_' ) );
      //### Parche temporal hasta sincronizar funcionalidad ren_asientos y ren_asientosPub
      if ( className == "ren" )
        className = funName.left( funName.indexOf( '_', 4 ) );
      //###
      while ( cl && !className.isEmpty() && USTR( cl->identifier() ) != className )
        cl = cl->base();
      if ( cl ) {
        cl = cl->base();
        if ( cl ) {
          cl = v.resolveMember( uBaseIdent, &mem, cl, &offset );
          if ( !cl ) {
            mem.setWritable( FALSE );
            QSReference ref( v, mem, cl );
            ref.setIdentifier( QSLexer::noop_ );
            return ref;
          }
        }
      }
  ...
  if ( offset ) {
    ...
    env->throwError( QString( "Trying to access undefined member '%1'" ).arg( ustr ) );
    return QSReference();
  }
...
QSObject QSAccessorNode2::rhs( QSEnv *env ) const
{
  ...
  if ( cl && mem.isDefined() ) {
    ...
    if ( obj.isUndefined() && mem.type() == QSMember::Identifier ) {
      ...
      return env->throwError( QString( "Trying to access undefined member '%1'" ).arg( USTR( ident ) ) );
    }
    updateErrorline();
    return obj;
  }
...
QSObject QSNewExprNode::evaluate( QSEnv *env )
{
  ...
  if ( cl == env->funcRefClass() ) {
    ...
    if ( !type ) {
      ...
      return throwError( env,
                         TypeError,
                         QString( "Cannot instantiate function '%1'" )
    ...
    return throwError( env,
                       TypeError,
                       QString( "Cannot instantiate object of type '%1'" )
  ...
  if ( !type->isAbstract() ) {
    ...
    env->throwError( TypeError,
                     QString( "Cannot instantiate abstract class '%1'" )
                     .arg( type->identifier() ),
                     lineNo() );
  }
  updateErrorline();
...
QSObject QSFunctionCallNode::rhs( QSEnv *env ) const
{
  ...
  if ( ref.identifier() == QSLexer::noop_ ) {
    QString msg =
      QLatin1String( "AbanQ : Member not defined in base class" );
#ifdef QSDEBUGGER
  ...
  if ( cl == env->funcRefClass() ) {
    ...
    QString msg =
      QString( "Undefined member function '%1' for object '%2' of type: '%3'" )
      .arg( !ref.identifier() ? QLatin1String( "unknown" ) : USTR( ref.identifier() ) )
      .arg( base.toString() )
      .arg( base.objectType() ? base.objectType() ->name() : QLatin1String( "undefined" ) );
    return env->throwError( TypeError, msg, lineNo() );
    ...
    if ( !cl->isExecutable() ) {
      ...
      QString msg =
        QString( "'%1' undefined or not a function" ).arg( USTR( mem.name() ) );
      return env->throwError( TypeError, msg, lineNo() );
    }
  }
  ...
  if ( env->stackDepth() > QS_MAX_STACK ) {
    ...
    if ( argList )
      delete argList;
    return env->throwError( RangeError, QLatin1String( "Stack overflow" ), lineNo() );
  }
#endif
...
void QSFunctionCallNode::steppingInto( bool in, QSEnv *env ) const
{
  ...
  if ( in ) {
    ...
    static_cast<QSFunctionCallNode *>( const_cast<QSFunctionCallNode *>( this ) ) ->previousMode = dbg->mode();
...
QSObject QSRelationalNode::rhs( QSEnv *env ) const
{
  ...
  if ( oper == OpLess || oper == OpGreaterEq ) {
    ...
    // ### Q_EMIT runtime warning about deprecated instanceof operator ?
    // ### without prototype support it doesn't do the right thing anyway
    if ( v2.objectType() ->name() == QLatin1String( "FactoryObject" ) ) {
      QSWrapperShared * s = static_cast<QSWrapperShared *>( v1.shVal() );
  ...
  if ( oper == OpEqual ) {
    ...
    if ( !ref.isWritable() ) {
      ...
      return env->throwError( ReferenceError,
                              QLatin1String( "Left hand side value is not writable" ),
                              lineNo() );
    }
    ref.assign( v2 );
    updateErrorline();
...
QSObject QSTypeOfNode::rhs( QSEnv *env ) const
{
  ...
  if ( ref.isReference() ) {
    if ( !ref.isDefined() )
      return env->createString( QSLexer::undefined_ );
    v = ref.dereference();
  } else
    v = ref.base();
...
QSObject QSForInNode::executeStatement( QSEnv *env )
{
  ...
  if ( lexpr ) {
    ...
    if ( !ref.isWritable() ) {
      ...
      QSObject e = env->throwError( TypeError, QLatin1String( "Non-writable index" ), lineNo() );
      return e;
    }
    base = ref.base();
    member = ref.member();
...
QSObject QSDoWhileNode::execute( QSEnv *env )
{
  ...
  do {
    ...
    if ( env->isContinueMode() && env->isCurrentLabelValid() ) {
      ...
      if ( env->currentLabel() > 0 )                                                                                           // Clean up if no label node is known
        env->setExecutionMode( QSEnv::Normal );
      result = value;
      break;
    } else if ( !env->isNormalMode() ) {
...
QSObject QSContinueNode::execute( QSEnv *env )
{
  ...
  if ( !env->containsLabel( ident ) ) {
    ...
    return env->throwError( SyntaxError,
                            QString( "Label %1 not found in containing block" )
                            .arg( USTR( ident ) ),
                            lineNo() );
  }
...
QSObject QSWithNode::executeStatement( QSEnv *env )
{
  ...
  if ( !v.isObject() ) {
    ...
    return env->throwError( ReferenceError,
                            QString( "Evaluation of with statement is not an object, "
                                     "was '%1' of type %2" )
                            .arg( v.toString() )
                            .arg( v.isValid() ? v.typeName() : QLatin1String( "invalid" ) ),
                            lineNo() );
  }
  env->pushScope( v );
...
QSObject QSLabelNode::execute( QSEnv *env )
{
  if ( env->containsLabel( label ) ) {
    ...
    env->throwError( SyntaxError,
                     QString( "Duplicated label %1" ).arg( USTR( label ) ),
                     lineNo() );
...
QSFunctionBodyNode::QSFunctionBodyNode( QSSourceElementsNode *s, QSLexer * l )
    : source( s ), scopeDef( 0 ), lx( l )
{
  idx = QSLexer::count();
#ifdef QSDEBUGGER
...
QSParameterNode* QSParameterNode::append( uint i, QSTypeNode *t )
{
...
QSProgramNode::QSProgramNode( QSSourceElementsNode *s, QSLexer * l )
    : QSFunctionBodyNode( s, l )
{
  lx->setLastNode( this );
}
...
QSObject QSTypedVarNode::rhs( QSEnv *env ) const
{
  if ( type ) {
    ...
    if ( !cl.isValid() ) {
      ...
      return env->throwError( QString( "Type '%1' is undefined" ).arg( type->identifier() ) );
    }
    QSList empty;
...
{
  QString errMsg;
  env->engine() ->requestPackage( USTR( package ), errMsg );
  if ( errMsg.isNull() )
    return QSObject();
  else {
