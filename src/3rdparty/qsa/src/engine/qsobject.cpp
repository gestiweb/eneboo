...
double NaN { const double *ret = reinterpret_cast<const double *>(NaN_Bytes); return *ret; }
double Inf { const double *ret = reinterpret_cast<const double *>(Inf_Bytes); return *ret; }
...
{
    Q_ASSERT( clss );
    Q_ASSERT( clss->name() != QLatin1String( "Type" ) );
}
...
QSObject::QSObject( const QSObject &o )
{
    ...
    if ( clss )
        clss->ref( static_cast<QSObject *>( this ) );
}

QSObject& QSObject::operator=( const QSObject &o )
{
    if ( o.clss )
        o.clss->ref( static_cast<QSObject *>( const_cast<QSObject *>( & o ) ) );
    if ( clss )
        clss->deref( static_cast<QSObject *>( this ) );
    clss = o.clss;
    val = o.val;
...
{
    static_cast<QSObject *>( this ) ->clss = c;
}
...
uint QSObject::toUString() const
{
    return objectType() ->toUString( this );
}
...
QSObject QSObject::getQualified( uint name ) const
{
    QSObject obj = *this;
    QString s( USTR( name ) );
...
// ECMA 8.6.2.1
QSObject QSObject::get( uint p ) const
{
    return objectType() ->get( this, p );
}

// ECMA 8.6.2.2
void QSObject::put( uint p, const QSObject& v )
{
    objectType() ->put( this, p, v );
}

// provided for convenience.
void QSObject::put( uint p, double d )
{
    put( p, env() ->createNumber( d ) );
}

// provided for convenience.
void QSObject::put( uint p, int i )
{
    put( p, env() ->createNumber( i ) );
}

// provided for convenience.
void QSObject::put( uint p, unsigned int u )
{
    put( p, env() ->createNumber( u ) );
}

// ECMA 8.6.2.4
bool QSObject::hasProperty( uint p ) const
{
    return objectType() ->hasProperty( this, p );
}

// ECMA 8.6.2.5
bool QSObject::deleteProperty( uint p )
{
    QSMember mem;
...
*/
const QSClass* QSObject::resolveMember( uint name,
                                        QSMember *mem,
                                        const QSClass *owner,
                                        int *offset ) const
{
...
void QSWritable::setProperty( uint n, const QSProperty &p )
{
    if ( props ) {
        ( *props ) [ n ] = p;
    } else {
...
QSProperty *QSWritable::reference( uint n ) const
{
    if ( props ) {
...
bool QSWritable::hasProperty( uint n ) const
{
...
void QSWritable::invalidate()
{
    if ( !props )
        return ;
    QMap<uint, QSProperty>::Iterator it = props->begin();
    QMap<uint, QSProperty>::Iterator end = props->end();
    while ( it != end ) {
        ( *it ).invalidate();
        ++it;
    }
...
QSObject Imp::defaultValue( const QSObject *o, Type hint ) const
{
    ...
    Imp *that = static_cast<Imp *>( this );
    if ( obj.isExecutable() ) { // spec says "not primitive type" but ...
        FunctionImp * f = static_cast<FunctionImp *>( obj.imp() );
        QSObject t( that );
...
void QSObject::setVal( uint v, bool )
{
    REF_USTR( v );
    val.str = v;
}
