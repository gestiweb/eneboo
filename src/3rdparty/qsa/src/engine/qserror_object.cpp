...
{
    return QLatin1String( "Error: " ) + errorMessage( obj );
}
...
#ifdef QSDEBUGGER
QSObject QSErrorClass::construct( ErrorType e, const QString &msg, int ln ) const
#else
QSObject QSErrorClass::construct( ErrorType, const QString &, int ) const
#endif
{
    QSErrorShared * error = 0;
#ifdef QSDEBUGGER
    error = new QSErrorShared( e, msg, ln );
...
ErrorType QSErrorClass::errorType( const QSObject *objPtr )
{
    ...
    return static_cast<QSErrorShared *>( objPtr->shVal() ) ->type;
}

QString QSErrorClass::errorName( const QSObject *objPtr )
{
    Q_ASSERT( objPtr->objectType() == objPtr->objectType() ->env() ->errorClass() );
    Q_ASSERT( static_cast<QSErrorShared *>( objPtr->shVal() ) ->type >= 0 );
    Q_ASSERT( static_cast<QSErrorShared *>( objPtr->shVal() ) ->type <= 8 );
    return QLatin1String( errNames[ static_cast<QSErrorShared *>( objPtr->shVal() ) ->type ] );
}

QString QSErrorClass::errorMessage( const QSObject *objPtr )
{
    Q_ASSERT( objPtr->objectType() == objPtr->objectType() ->env() ->errorClass() );
    return static_cast<QSErrorShared *>( objPtr->shVal() ) ->message;
}

int QSErrorClass::errorLine( const QSObject *objPtr )
{
    Q_ASSERT( objPtr->objectType() == objPtr->objectType() ->env() ->errorClass() );
    return static_cast<QSErrorShared *>( objPtr->shVal() ) ->line;
}

void QSErrorClass::setErrorLine( QSObject *objPtr, int line )
{
    Q_ASSERT( objPtr->objectType() == objPtr->objectType() ->env() ->errorClass() );
    static_cast<QSErrorShared *>( objPtr->shVal() ) ->line = line;
}
...
{
    Q_ASSERT( objPtr->objectType() == objPtr->objectType() ->env() ->errorClass() );
    return static_cast<QSErrorShared *>( objPtr->shVal() ) ->sid;
}
#endif
