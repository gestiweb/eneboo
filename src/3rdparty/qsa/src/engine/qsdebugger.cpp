...
QString Debugger::varInfo( const QString & )
{
    ...
    QSObject obj;
    // resolve base
    if ( sub == QLatin1String( "||Global||" ) ) {
        obj = env() ->globalObject();
    } else if ( sub == QLatin1String( "||Activation||" ) ) {
        obj = env() ->currentScope();
    } else if ( sub == QLatin1String( "this" ) ) {
        obj = env() ->thisValue();
    } else {
...
{
    if ( !eng || ident == QLatin1String( "this" ) )
        return false;
    ...
    while ( dot > 0 ) {
        ...
        sub = ident.mid( olddot + 1, dot - olddot - 1 );
        if ( !base.hasProperty( HASH_USTR( sub ) ) )
            return false;
        base = base.get( HASH_USTR( sub ) );
    }

    base.put( NEW_USTR( sub ), value );
