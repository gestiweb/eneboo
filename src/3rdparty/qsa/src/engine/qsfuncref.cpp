...
QSObject QSFuncRefClass::construct( const QSList &args ) const
{
    ...
    if ( argsSize == 0 ) {
        ...
        for ( int k = 1; k < argsSize - 1; ++k )
            p += QLatin1String( "," ) + args[ k ].toString();
    ...
    if ( parseError ) {
        if ( progNode && progNode->deref() )
            delete progNode;
        return env() ->throwError( SyntaxError, QLatin1String( "Syntax error in function body" ) );
    }

    QSFunctionScopeClass *scope = new QSFunctionScopeClass( env() ->objectClass() );
    QSFunctionBodyNode * bodyNode = lx->lastNode();
    ...
    while ( i < len ) {
        ...
        if ( QSLexer::isIdentLetter( c->unicode() ) ) {  // else error
            ...
            if ( i == len ) {
                ...
                int index = scope->addVariableMember( NEW_USTR( param ), AttributeNone );
        ...
        return env() ->throwError( SyntaxError, QLatin1String( "Syntax error in parameter list" ) );
    }
...
uint QSFuncRefClass::toUString( const QSObject * ) const
{
    return QSLexer::function_; // ###
}
                                 ...
                                 const QSMember & ) const
{
    ...
    QSReferenceData * data = static_cast<QSReferenceData *>( objPtr->shVal() );
    QSObject base = data->base;
    QSMember bmem = data->member;

    env() ->pushScopeBlock();
    for ( ScopeChain::const_iterator it = data->context.begin(); it != data->context.end(); ++it )
        env() ->pushScope( *it );
        ...
        const QSMember &mem ) const
{
    ...
    ScopeChain context;
    for ( ScopeChain::const_iterator it = chain.begin(); it != chain.end(); ++it )
...
{
    if ( ref.isFunction() ) {
        return static_cast<QSReferenceData *>( ref.shVal() ) ->member;
    }
...
{
    if ( ref.isFunction() ) {
        return static_cast<QSReferenceData *>( ref.shVal() ) ->base;
    }
...
{
    if ( ref.isFunction() ) {
        return static_cast<QSReferenceData *>( ref.shVal() ) ->context;
    }
