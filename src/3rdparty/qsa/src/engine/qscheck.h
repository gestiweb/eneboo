...
public:
    ...
    uint name() const { return cl->identifier(); }
...
public:
    ...
    void leaveClass();
    bool insideClass(uint name) const;
    void registerType(QSClass *c);
    QSClass *typeInfo(uint name) const;

    void enterFunction(QSFunctionScopeClass *c);
    void leaveFunction();

    void pushLabel(uint label) { lablist.push_front(label); }
    void popLabel() { lablist.pop_front(); }
    void setLabel(uint label);
    uint currentLabel() const;
    bool seenLabel(uint label) const;
    void clearLabel();

    void enterBlock(QSBlockScopeClass *cl);
    void leaveBlock();

    void enterLoop(uint label = 0);
    void leaveLoop();
...
private:
    ...
    uint lastlab;		// last label seen
    QList<uint> lablist;	// list of label for iteration statements
    //    QStringList flist;
    QSTypeMap tmap;
