...
{
    return rep->evaluate( QLatin1String( code ) );
}
...
bool QSEngine::evaluateByteCode( const QSObject &thisV, const QByteArray &code, int linezero )
{
    return rep->evaluateByteCode( code, thisV.isValid() ? &thisV : 0, linezero );
}

bool QSEngine::evaluateByteCode( const QSObject &thisV, QDataStream & in, int linezero )
{
    return rep->evaluateByteCode( in, thisV.isValid() ? &thisV : 0, linezero );
}
...
QMutex *qsa_lexer_mutex()
{
    return qsa_engine_mutex();
}
                            ...
                            bool deleteNodes )
{
    ...
    int parseError = qsyyparse( rep->lexer() );
    QSNode *lastNode = rep->lexer() ->lastNode();
    bool success = true;

    if ( parseError ) {
        rep->errType = QSErrParseError;
        int l = rep->lexer() ->lineNo();
        rep->errLines.append( l );
        rep->errMsgs.append( QLatin1String( "Parse Error: " )
                             + qs_format_error( rep->lexer() ->errorMessage() ) );
        success = false;
    }
...
void QSEngine::enableDebug()
{
    ...
    env() ->globalClass() ->addMember( REG_USTR( QLatin1String( "debug" ) ), debug );

    QSMember print( &printFunction );
    env() ->globalClass() ->addMember( REG_USTR( QLatin1String( "print" ) ), print );
}
...
{
    Q_EMIT packageRequest( package, err );
}
