...
{
    addMember( REG_USTR( QLatin1String( "toString" ) ), QSMember( &QSObjectClass::toStringScript ) );
    addMember( REG_USTR( QLatin1String( "valueOf" ) ), QSMember( &QSObjectClass::valueOf ) );
}

QString QSObjectClass::toString( const QSObject *obj ) const
{
    return QLatin1String( "[object " ) + obj->typeName() + QLatin1String( "]" );
}
...
{
    QString str( QLatin1String( "[object " ) + env->thisValue().typeName() + QLatin1String( "]" ) );
    return env->createString( NEW_USTR( str ) );
}
