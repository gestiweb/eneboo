...
#else
QuickInterpreter::QuickInterpreter( bool )
#endif
        : toplevel( 0 )
{
...
{
    return static_cast<QuickInterpreter *>( e->engine() );
}
...
void QuickInterpreter::timerEvent( QTimerEvent *e )
{
    ...
    QString func_name = USTR( QSFuncRefClass::refMember( function ).name() );
...
QVariant QuickInterpreter::convertToArgument( const QSObject &o )
{
    ...
    if ( cl->name() == QLatin1String( "QObject" ) ) {
        QSWrapperShared * shared = static_cast<QSWrapperShared *>( o.shVal() );
...
QSObject QuickInterpreter::wrap( QObject *o )
{
    ...
    QSUserData *udata = static_cast<QSUserData *>( o->userData( userDataId() ) );
...
void QuickInterpreter::init()
{
    ...
    staticGlobals << QLatin1String( "NaN" )
    ...
    global.put( QSLexer::application_, appClass->createWritable() );
    ...
    //     //
    env() ->globalClass() ->addMember( REG_USTR( QLatin1String( "connect" ) ), QSMember( qsa_connect ) );
    env() ->globalClass() ->addMember( REG_USTR( QLatin1String( "disconnect" ) ), QSMember( qsa_disconnect ) );
    env() ->globalClass() ->addMember( REG_USTR( QLatin1String( "startTimer" ) ), QSMember( qsStartTimer ) );
    env() ->globalClass() ->addMember( REG_USTR( QLatin1String( "killTimer" ) ), QSMember( qsKillTimer ) );
    env() ->globalClass() ->addMember( REG_USTR( QLatin1String( "killTimers" ) ), QSMember( qsKillTimers ) );

    QMap<uint, QObject*> statDescr = factory->staticDescriptors();
    QMap<uint, QString> instDescr = factory->instanceDescriptors();

    QList<uint> features = instDescr.keys();
    for ( QList<uint>::ConstIterator it = features.begin();
            it != features.end(); ++it ) {
        if ( env() ->globalClass() ->definedMembers() ->contains( *it ) ) {
            qWarning( "QSObjectFactory: Trying to register existing class: '%s'", USTR( *it ).toLatin1().constData() );
            continue;
        }
        QSObject staticInst;
        ...
        QSObjectConstructor *constr = new QSObjectConstructor( objClass, *it );
        QSFactoryObjectProxy *ptype = new QSFactoryObjectProxy( env() ->typeClass(), staticInst, constr );
    ...
    for ( QMap<uint, QObject*>::ConstIterator sit = statDescr.begin();
            sit != statDescr.end(); ++sit ) {
        if ( env() ->globalClass() ->definedMembers() ->contains( sit.key() ) ) {
            qWarning( "QSObjectFactory: Trying to register existing class: '%s'", USTR( sit.key() ).toLatin1().constData() );
            continue;
        }
        QSObject staticInst;
...
void QuickInterpreter::addTopLevelObject( QObject *o )
{
    ...
    QSObject global = env() ->globalObject();
    static_cast<QSClass *>( const_cast<QSClass *>( global.objectType() ) ) ->deleteMember( HASH_USTR( o->objectName() ) );
    ...
    env() ->globalObject().put( NEW_USTR( context_name ), wrap( o ) );
}
...
void QuickInterpreter::setTopLevelObjects( QObjectList *l )
{
    ...
    for ( int i = 0; i < toplevel->size(); ++i ) {
        ...
        connect( o, SIGNAL( destroyed( QObject * ) ),
                 this, SLOT( topLevelDestroyed( QObject * ) ) );
        global.put( NEW_USTR( o->objectName() ), wrap( o ) );
...
QVariant QuickInterpreter::executeByteCode( QObject *obj, const QByteArray &code,
        const QString &name )
{
    int sourceId = 0;
#ifdef QSDEBUGGER
    ...
    QSEngine::evaluateByteCode( t, code );
...
QVariant QuickInterpreter::executeByteCode( QObject *obj, QDataStream & in,
        const QString &name )
{
    int sourceId = 0;
#ifdef QSDEBUGGER
    ...
    QSEngine::evaluateByteCode( t, in );
...
void QuickInterpreter::setVariable( QObject *context, const QString &func, const QVariant &value )
{
    ...
    if ( context )
        wrap( context ).put( NEW_USTR( func ), val );
    else
        env() ->globalObject().put( NEW_USTR( func ), val );
}
...
QVariant QuickInterpreter::variable( QObject *context, const QString &varName )
{
    ...
    return convertToArgument( obj.getQualified( HASH_USTR( varName ) ) );
}
...
void QuickInterpreter::cleanType( QString &type )
{
    ...
    if ( type.left( 5 ) == QLatin1String( "const" ) )
        type.remove( 0, 5 );
    ...
    else if ( type == QLatin1String( "int" ) || type == QLatin1String( "uint" ) ||
              type == QLatin1String( "long" ) || type == QLatin1String( "ulong" ) ||
              type == QLatin1String( "double" ) || type == QLatin1String( "float" ) )
...
bool QuickInterpreter::queryDispatchObjects( QObject *obj, QVector<QObject *> *result )
{
    ...
    while ( meta ) {
        factory->constructInterface( meta->className(), static_cast<void *>( obj ), result );
...
{
    QSClassList classList( env() ->classes() );
    QStringList lst;
    for ( QSClassList::const_iterator it = classList.begin(); it != classList.end(); ++it ) {
        if ( ( *it ) ->asClass() )
            lst << USTR( ( *it ) ->asClass() ->identifier() );
    }
    return lst;
}
...
QSObject QuickInterpreter::object( const QString &name ) const
{
    ...
    Global *global = static_cast<Global *>( & g ); // ### ugly
    QSObject obj;
    if ( name.isEmpty() ) {
        obj = g;
    } else {
        ...
        if ( p == -1 )
            obj = global->get( HASH_USTR( name ) );
        else
            obj = global->getQualified( HASH_USTR( name ) );
    }
    return obj;
}
...
QStringList QuickInterpreter::functionsOf( QSObject &obj, bool includeSignature, bool includeNative, bool includeMemberFunctions ) const
{
    ...
    QSMemberMap mmap = objType == env() ->typeClass() && includeMemberFunctions
                       ? static_cast<const QSTypeClass *>( objType ) ->allMembers( &obj )
                       : objType->members( &obj );
...
QStringList QuickInterpreter::classesOf( QSObject &obj ) const
{
    ...
    for ( int i = 0; i < cls->numStaticVariables(); ++i ) {
        ...
        if ( o.isA( env() ->typeClass() ) && QSTypeClass::classValue( &o ) ->asClass() )
            lst << USTR( QSTypeClass::classValue( &o ) ->identifier() );
    }
    return lst;
}
...
static bool hasMember( QSEnv *env, const QString &function, QSMember::Type type )
{
    ...
    QSMember member;
    QStringList names = function.split( QLatin1String( "." ) );
    ...
    for ( QStringList::ConstIterator it = names.begin(); it != names.end(); ++it, --nameCount ) {
        if ( nameCount == 1 ) {
            if ( o.objectType() == env->typeClass() )
                return QSTypeClass::classValue( &o ) ->member( 0, HASH_USTR( *it ), &member )
                       && member.type() == type;
            else if ( o.objectType() ->member( &o, HASH_USTR( *it ), &member ) )
                return o.objectType() ->member( 0, HASH_USTR( *it ), &member ) && member.type() == type;
        } else {
            o = o.get( HASH_USTR( *it ) );
            if ( !o.isValid() )
                return false;
        }
    }
                        ...
                        void *argumentList[] )
{
    ...
    QString func_name = USTR( QSFuncRefClass::refMember( connection.function_ref ).name() );
...
static QSObject qsa_connect( QSEnv *env )
{
    ...
    if ( args->size() == 3 ) {
        ...
        if ( !function_ref.isFunction() )
            function_ref = env->resolveValue( INDEX_USTR( args->at( 2 ).toString() ) );
...
QSObject qsStartTimer( QSEnv *env )
{
    ...
    QString msg = QLatin1String( "Can only install functions as event handler" );
