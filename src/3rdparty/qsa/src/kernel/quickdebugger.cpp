...
{
    QString simple = var;
    simple.replace( QRegExp( QLatin1String( "\\[" ) ), QLatin1String( "." ) );
    simple.replace( QRegExp( QLatin1String( "\\]" ) ), QLatin1String( "" ) );
...
bool QuickDebugger::setVariable( const QString &var, const QString &val )
{
    ...
    bool ok;
    double d;
    if ( v == QLatin1String( "null" ) )
        value = env() ->createNull();
    else if ( v == QLatin1String( "undefined" ) )
        value = env() ->createUndefined();
    else if ( v == QLatin1String( "true" ) )
        value = env() ->createBoolean( true );
    else if ( v == QLatin1String( "false" ) )
        ...
        if ( ( c0 == '"' || c0 == '\'' ) && v[ ( int ) v.length() - 1 ] == c0 )
            value = env() ->createString( env() ->lexer() ->newUstr( v.mid( 1, v.length() - 2 ) ) );
        else
            return false;
...
void QuickDebugger::callEvent( const QString &fn, const QString & )
{
    ...
    cStack.prepend( sf );
    Q_EMIT stackChanged( cStack.count() );
}
...
void QuickDebugger::storeExceptionStack()
{
    ...
    *exceptionScope = env() ->scope();
    ...
    if ( exceptionStack.count() == 0 ) {
        QuickDebuggerStackFrame sf( QLatin1String( "No function" ), lineNumber(), sourceId() );
