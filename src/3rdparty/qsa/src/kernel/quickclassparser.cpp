...
QString QuickClassParser::legalChars = QLatin1String( "abcdefghijklmnopqrstufvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890_/*" );
...
void QuickClassParser::parse( const QString &code )
{
    ...
    for ( pos = 0; pos < ( int ) formCode.length(); ++pos ) {
        if ( legalChars.indexOf( formCode[ pos ] ) == -1 )
            ...
            expr += formCode[ pos ];
        if ( expr == QLatin1String( "var" )
                || expr == QLatin1String( "const" )
                || expr == QLatin1String( "static" ) ) {
            int i = pos + 1;
            while ( formCode[ i ] == ' ' || formCode[ i ] == '\t' )
                ++i;
            if ( formCode.mid( i, 8 ) != QLatin1String( "function" ) ) {
                ++pos;
                parseVariableDecl( expr );
            }
            expr = "";
            continue;
        } else if ( expr == QLatin1String( "private" )
                    || expr == QLatin1String( "protected" )
                    || expr == QLatin1String( "public" ) ) {
            lastAccess = expr;
            expr = QLatin1String( "" );
            continue;
        } else if ( expr == QLatin1String( "function" )
                    || expr == QLatin1String( "constructor" ) ) {
            ++pos;
            parseFunction( expr == QLatin1String( "function" )
                           ? functionLength
                           : constructorLength );
            expr = "";
            continue;
        } else if ( expr == QLatin1String( "connect" ) ) {
            ++pos;
            parseConnections();
            expr = "";
            continue;
            ...
            expr = "";
            continue;
        } else if ( expr == QLatin1String( "class" ) ) {
        ...
        if ( state == Body ) {
            if ( !open ) {
                break;
                        ...
                        && body.right( functionLength ) == QLatin1String( "function" ) ) {
    ...
    for ( ; pos < ( int ) formCode.length(); ++pos ) {
        if ( formCode[ pos ] == ';' ) {
            ...
            if ( var[ ( int ) var.length() - 1 ] != ';' )
                var += QLatin1String( ";" );
            currClass->variables << QString( expr + QLatin1String( " " ) + var );
            return ;
        }
        var += formCode[ pos ];
    }
