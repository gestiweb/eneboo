...
{
    addMember( REG_USTR( QLatin1String( "width" ) ), QSMember( QSMember::Custom, Width,
               AttributeNonWritable ) );
    addMember( REG_USTR( QLatin1String( "height" ) ), QSMember( QSMember::Custom, Height,
               AttributeNonWritable ) );
    addMember( REG_USTR( QLatin1String( "rect" ) ), QSMember( QSMember::Custom, Rect,
               AttributeNonWritable ) );
    addMember( REG_USTR( QLatin1String( "size" ) ), QSMember( QSMember::Custom, Size,
               AttributeNonWritable ) );
    addMember( REG_USTR( QLatin1String( "depth" ) ), QSMember( QSMember::Custom, Depth,
               AttributeNonWritable ) );
    addMember( REG_USTR( QLatin1String( "isNull" ) ), QSMember( &isNull ) );
    addMember( REG_USTR( QLatin1String( "fill" ) ), QSMember( &fill ) );
    addMember( REG_USTR( QLatin1String( "resize" ) ), QSMember( &resize ) );
    addMember( REG_USTR( QLatin1String( "load" ) ), QSMember( &load ) );
    addMember( REG_USTR( QLatin1String( "save" ) ), QSMember( &save ) );
}

QPixmap *QSPixmapClass::pixmap( const QSObject *obj ) const
{
    assert( obj->objectType() == this );
    return &( static_cast<PixmapShared *>( obj->shVal() ) ->pixmap );
}
...
QSObject QSPixmapClass::isNull( QSEnv *env )
{
    ...
    const QSPixmapClass *pac = static_cast<const QSPixmapClass *>( t.objectType() );
    QSObject result;
...
{
    if ( env->numArgs() != 1 ) {
        env->throwError( QString( QLatin1String( "Pixmap.fill() called with %1 arguments. 1 arguments expected." ) ).
                         arg( env->numArgs() ) );
        return ;
    }
    ...
    if ( !a0.isA( "Color" ) ) {
        env->throwError( QString( QLatin1String( "Pixmap.fill() called with an argument of type %1. "
    ...
    const QSPixmapClass *pac = static_cast<const QSPixmapClass *>( t.objectType() );
    QPixmap *pix = pac->pixmap( &t );
    pix->fill( *( static_cast<const QSColorClass *>( a0.objectType() ) ) ->color( &a0 ) );
    return ;
}
...
void QSPixmapClass::resize( QSEnv *env )
{
    if ( env->numArgs() < 1 || env->numArgs() > 2 ) {
        env->throwError( QString( QLatin1String( "Pixmap.resize() called with %1 arguments. 1 or 2 arguments expected." ) ).
                         arg( env->numArgs() ) );
        return ;
    }
    ...
    if ( env->numArgs() == 1 ) {
        QSObject a0 = env->arg( 0 );
        if ( !a0.isA( "Size" ) ) {
            env->throwError( QString( QLatin1String( "Pixmap.resize() called with an argument of type %1. "
                                      "Type Size is expected" ) ).
                             arg( a0.typeName() ) );
            return ;
        }
        QSize size = *( static_cast<const QSSizeClass *>( a0.objectType() ) ) ->size( &a0 );
        w = size.width();
        h = size.height();
    } else {
        if ( !env->arg( 0 ).isA( "Number" ) || !env->arg( 1 ).isA( "Number" ) ) {
            env->throwError( QString( QLatin1String( "Pixmap.resize() called with arguments of type %1 and %2. "
...
{
    if ( env->numArgs() != 1 ) {
        env->throwError( QString( QLatin1String( "Pixmap.load() called with %1 arguments. 1 argument expected." ) ).
                         arg( env->numArgs() ) );
        return ;
    }

    if ( !env->arg( 0 ).isString() ) {
        env->throwError( QString( QLatin1String( "Pixmap.load() called with an argument of type %1. "
...
{
    if ( env->numArgs() < 1 || env->numArgs() > 2 ) {
        env->throwError( QString( QLatin1String( "Pixmap.save() called with %1 arguments. 1 or 2 argument expected." ) ).
                         arg( env->numArgs() ) );
        return ;
    }
    ...
    QPixmap *pix = pac->pixmap( &t );
    if ( !env->arg( 0 ).isString() ) {
        env->throwError( QString( QLatin1String( "Pixmap.save() called with an argument of type %1. "
                                  "Type String is expeced" ) ).
                         arg( env->arg( 0 ).typeName() ) );
        return ;
    }
