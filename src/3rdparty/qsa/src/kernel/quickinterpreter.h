...
public:
  ...
  QVariant executeByteCode( QObject *_this, const QByteArray &code,
                            const QString &scriptName );
  QVariant executeByteCode( QObject *_this, QDataStream & in,
                            const QString &scriptName );
  ...
  QSAConnection scriptSlot( int id ) const
  {
    return m_script_connections.value( id );
  }

  QHash<int, QSObject> *timers()
  {
    return & m_timers;
  }
