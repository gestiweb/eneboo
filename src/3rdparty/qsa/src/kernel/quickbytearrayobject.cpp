...
{
    // custom members
    addMember( REG_USTR( QLatin1String( "length" ) ), QSMember( QSMember::Custom, Length, AttributeNonWritable ) );
    addMember( REG_USTR( QLatin1String( "size" ) ), QSMember( QSMember::Custom, Size, AttributeNonWritable ) );

    // member functions
    addMember( REG_USTR( QLatin1String( "toString" ) ), QSMember( &QSByteArrayClass::toStringScript ) );
    addMember( REG_USTR( QLatin1String( "charAt" ) ), QSMember( &QSByteArrayClass::charAt ) );
}

QByteArray *QSByteArrayClass::byteArray( const QSObject *obj ) const
{
    assert( obj->objectType() == this );
    return &( static_cast<ByteArrayShared *>( obj->shVal() ) ->byteArray );
}
...
{
    return env->createString( env->lexer() ->newUstr( env->thisValue().toString() ) );
}

QSObject QSByteArrayClass::charAt( QSEnv *env )
{
    const QSByteArrayClass * bac = static_cast<const QSByteArrayClass *>( env->thisClass() );
    QSObject obj = env->thisValue();
    QByteArray *ba = bac->byteArray( &obj );
    if ( env->numArgs() > 0 ) {
        int pos = env->arg( 0 ).toInteger();
        if ( pos >= 0 && pos < ( int ) ba->size() )
            return env->createString( env->lexer() ->newUstr( QString( ( QChar ) ba->data() [ pos ] ) ) );
    }
