...
QSWrapperShared *QSWrapperClass::shared( const QSObject *obj ) const
{
  ...
  return static_cast<QSWrapperShared *>( obj->shVal() );
}
...
void QSWrapperClass::deref( QSObject *o ) const
{
  ...
  o->shVal() ->deref();
  if ( o->shVal() ->count == 0 ) {
    static_cast<QSWrapperClass *>( const_cast<QSWrapperClass *>( this ) ) ->invalidate();
    env() ->removeShared( o->shVal() );
    delete o->shVal();
    o->setVal( static_cast<QSShared *>( 0 ) );
  }
#endif
}

bool QSWrapperClass::member( const QSObject *objPtr, uint p,
                             QSMember *mem ) const
{
  ...
  QString uP( USTR( p ) );
  QByteArray member_name_ba = uP.toLatin1();
  const char *member_name = member_name_ba.data();
  int member_name_length = uP.length();
  ...
  for ( int i = objects.size() - 1; i >= 0; --i ) {
    ...
    for ( int child_index = 0; child_index < children.size(); ++child_index ) {
      if ( uP == children.at( child_index ) ->objectName() ) {
                                     ...
                                     const QSMember &mem ) const
{
  ...
  case PropertyType: {
      ...
      if ( !property.isReadable() )
        return env() ->throwError( QString( "Property %1 is not readable" ).arg( USTR( mem.name() ) ) );
      if ( !property.isScriptable() )
        return env() ->throwError( QString( "Property %1 is not scriptable" ).arg( USTR( mem.name() ) ) );
  ...
  case ObjectType: {
      ...
      Q_ASSERT( mem.name() == HASH_USTR( child->objectName() ) );
                            ...
                            const QSObject &val ) const
{
  ...
  case PropertyType: {
      ...
      if ( !meta_property.isWritable() )
        env() ->throwError( QString( "Property %1 is not writable" ).arg( USTR( mem.name() ) ) );
      if ( !meta_property.isScriptable() )
        env() ->throwError( QString( "Property %1 is not scriptable" ).arg( USTR( mem.name() ) ) );
...
QSEqualsResult QSWrapperClass::isEqual( const QSObject &a, const QSObject &b ) const
{
  ...
  const QSWrapperClass *aclass = static_cast<const QSWrapperClass *>( a.objectType() );
  const QSWrapperClass *bclass = static_cast<const QSWrapperClass *>( b.objectType() );
...
QObject *QSWrapperClass::object( const QSObject *obj )
{
  ...
  Q_ASSERT( static_cast<const QSWrapperClass *>( obj->objectType() ) ->objectVector( obj ) ->size() > 0 );
  return static_cast<const QSWrapperClass *>( obj->objectType() ) ->objectVector( obj ) ->at( 0 );
}


QSObject QSObjectConstructor::cast( const QSList & ) const
{
  env() ->throwError( QLatin1String( "Unable to perform cast to user defined object type '" ) + USTR( cname ) + QLatin1String( "'" ) );
  return QSObject();
}
...
bool QSFactoryObjectProxy::member( const QSObject *, uint n,
                                   QSMember *m ) const
{
...
void QSFactoryObjectProxy::write( QSObject *, const QSMember &mem,
                                  const QSObject &val ) const
{
  sinst.objectType() ->write(
    &( static_cast<QSFactoryObjectProxy *>( const_cast<QSFactoryObjectProxy *>( this ) ) ->sinst ), mem, val );
}

QSObject QSFactoryObjectProxy::invoke( QSObject *, const QSMember &mem ) const
{
  return static_cast<const QSWrapperClass *>( sinst.objectType() ) ->invoke(
           &( static_cast<QSFactoryObjectProxy *>( const_cast<QSFactoryObjectProxy *>( this ) ) ->sinst ), mem );
}

QSObjectConstructor::QSObjectConstructor( QSClass *b,
    uint className, Type t )
...
QSObject QSObjectConstructor::construct( const QSList &args ) const
{
  if ( type == Class ) {
    ...
    if ( !qobject_cast<QuickPtrDispatchObject *>( result[ 0 ] ) ) {
      ...
      if ( m_proxy ) {
        static_cast<QSWrapperShared *>( obj.shVal() ) ->creator = m_proxy;
      }
      return obj;
    }
    return interpreter() ->pointerClass() ->
           wrapPointer( static_cast<QuickPtrDispatchObject *>( result[ 0 ] ) );
  }
  return createUndefined();
}


bool QSObjectConstructor::member( const QSObject *, uint n, QSMember *m ) const
{
  extern const QMetaObject * qsa_query_meta_object( const QByteArray & name );
  const QMetaObject *metaObj = qsa_query_meta_object( USTR( cname ).toLatin1() );

  if ( !metaObj )
    metaObj = qsa_query_meta_object( ( QLatin1String( "QS" ) + USTR( cname ) ).toLatin1() );

  if ( !metaObj )
    return false;

  QByteArray matchName = USTR( n ).toLatin1();
...
void* QSPointerClass::pointer( const QSObject *obj ) const
{
  ...
  if ( objects->isEmpty() )
    return 0;
  return static_cast<QuickPtrDispatchObject *>( objects->at( 0 ) ) ->pointer();
}
...
QSVariantShared *QSVariantClass::shared( const QSObject *obj ) const
{
  ...
  return static_cast<QSVariantShared *>( obj->shVal() );
}
...
{
  return static_cast<QSVariantShared *>( shVal() );
}
...
static inline QSObject stringlist_to_qsarray( const QStringList &lst, QSEnv *env )
{
  ...
  for ( QStringList::ConstIterator it = lst.begin();
        it != lst.end(); ++it )
    array.put( NEW_USTRNUM( i++ ), env->createString( NEW_USTR( *it ) ) );
  return array;
}
...
void QSVariantShared::createObject( QuickInterpreter *ip )
{
  ...
  case QVariant::List: {
      ...
      for ( QList<QVariant>::ConstIterator it = lst.begin(); it != lst.end(); ++it ) {
        ...
        array.put( NEW_USTRNUM( i++ ), qsvar.isNative() ? qsvar.toNative() : qsvar );
      }
      iobj = array;
      break;
    }
  case QVariant::Map: {
      ...
      for ( QMap<QString, QVariant>::ConstIterator it = map.begin(); it != map.end(); ++it ) {
        QuickScriptVariant qsvar = QuickScriptVariant( ip, *it );
        obj.put( NEW_USTR( it.key() ), qsvar.isNative() ? qsvar.toNative() : qsvar );
      }
      iobj = obj;
      break;
    }
  case QVariant::String:
    iobj = env->createString( env->lexer() ->newUstr( var.toString() ) );
    break;
  default:
    iobj = QSObject(); // null !
    break;
...
QString QSVariantClass::toString( const QSObject *obj ) const
{
  ...
  case QVariant::Point: {
      ...
      return QString( QLatin1String( "(%1, %2)" ) ).arg( point.x() ).arg( point.y() );
    }
  case QVariant::Rect: {
      QRect rect = var->toRect();
      return QString( QLatin1String( "(%1, %2, %3, %4)" ) ).arg( rect.x() ).arg( rect.y() ).
             arg( rect.width() ).arg( rect.height() );
    }
  case QVariant::Size: {
      QSize size = var->toSize();
      return QString( QLatin1String( "(%1, %2)" ) ).arg( size.width() ).arg( size.height() );
    }
  case QVariant::Date:
                             ...
                             QSMember *m ) const
{
  ...
  return sh->iobj.objectType() ->member( &sh->iobj, HASH_USTR( n ), m );
}
...
QSMemberMap QSApplicationClass::members( const QSObject *o ) const
{
  ...
  if ( ( objects = interpreter() ->topLevelObjects() ) != 0 ) {
    for ( int i = 0; i < objects->size(); ++i ) {
      ...
      m.setType( QSMember::Custom );
      m.setOwner( this );
      m.setName( NEW_USTR( o->objectName() ) );
      m.setWritable( false );
      mmap[ m.name() ] = m;
    }
  }
...
bool QSApplicationClass::member( const QSObject *o, uint n,
                                 QSMember *m ) const
{
  if ( !interpreter() ->children().contains( USTR( n ) ) )       // a top level object ?
  ...
  for ( int slot_index = 0; slot_index < slot_indexes.size(); ++slot_index ) {
    ...
    if ( types.isEmpty() ) {
      ...
      for ( int i = 0; i < type_names.size(); ++i ) {
        ...
        if ( t.id == QMetaType::Void ) {
          if ( strcmp( t.name, "QMap<QString,QVariant>" ) == 0 )
            t.id = QVariant::Map;
          else if ( strcmp( t.name, "QList<QVariant>" ) == 0 )
            t.id = QVariant::List;
          else if ( strcmp( t.name, "qreal" ) == 0 )
            t.id = QVariant::Double;
        }
        types << t;
      }
    ...
    for ( ; pos < types.size() && match; ++pos ) {
      ...
      switch ( ti.id ) {
        ...
        // Boolean type
      case QMetaType::Bool:
        QSA2QT( bools, bool, arg.toBoolean(), booleanClass );

        // Integer types
      case QMetaType::Short:
        QSA2QT_NAN_CHECK( shorts, short, arg.toNumber(), numberClass );
      case QMetaType::UShort:
        QSA2QT_NAN_CHECK( ushorts, ushort, arg.toNumber(), numberClass );
      case QMetaType::Int:
        QSA2QT_NAN_CHECK( ints, int, arg.toNumber(), numberClass );
      case QMetaType::UInt:
        QSA2QT_NAN_CHECK( uints, uint, arg.toNumber(), numberClass );
      case QMetaType::Long:
        QSA2QT_NAN_CHECK( longs, long, arg.toNumber(), numberClass );
      case QMetaType::ULong:
        QSA2QT_NAN_CHECK( ulongs, ulong, arg.toNumber(), numberClass );

        // Float types
      case QMetaType::Float:
        QSA2QT( floats, float, arg.toNumber(), numberClass );
      case QMetaType::Double:
        QSA2QT( doubles, double, arg.toNumber(), numberClass );

        // Text and char types
      case QMetaType::Char:
        QSA2QT( chars, char, qsa_first_char( arg ), numberClass );
      case QMetaType::UChar:
        QSA2QT( uchars, uchar, qsa_first_char( arg ), numberClass );
      case QMetaType::QChar:
        QSA2QT( qchars, QChar, qsa_first_qchar( arg ), numberClass );
      case QMetaType::QString:
        QSA2QT( strings, QString, arg.toString(), stringClass );
      ...
      case QMetaType::VoidStar:
        if ( arg.isA( interpreter->pointerClass() ) )
          data[ pos ] =
            caching->voidptrs( static_cast<const QSPointerClass *>( arg.objectType() ) ->pointer( &arg ) );
      ...
      default: {
          ...
          if ( v->isValid() && ( int ) v->type() == ti_id ) {
            ...
            data[ pos ] = static_cast<void *>( const_cast<QVariant *>( v ) );
            ...
            if ( arg.objectType() == env->arrayClass() ) {
              ...
              for ( int ai = 0; ai < len; ++ai )
                tmp_list << ( int ) arg.get( HASH_USTRNUM( ai ) ).toNumber();
            ...
            if ( arg.objectType() == env->arrayClass() ) {
              ...
              int len = QSArrayClass::length( &arg );
              for ( int ai = 0; ai < len; ++ai ) {
                QSObject a = arg.get( HASH_USTRNUM( ai ) );
...
static QSObject qsa_pod_value( QSEnv *env, QMetaType::Type type, void *value )
{
  switch ( type ) {
  case QMetaType::Bool:
    return env->createNumber( *( static_cast<bool *>( value ) ) );
  case QMetaType::Short:
    return env->createNumber( *( static_cast<short *>( value ) ) );
  case QMetaType::UShort:
    return env->createNumber( *( static_cast<ushort *>( value ) ) );
  case QMetaType::Int:
    return env->createNumber( *( static_cast<int *>( value ) ) );
  case QMetaType::UInt:
    return env->createNumber( *( static_cast<uint *>( value ) ) );
  case QMetaType::Float:
    return env->createNumber( *( static_cast<float *>( value ) ) );
  case QMetaType::Double:
    return env->createNumber( *( static_cast<double *>( value ) ) );
  case QMetaType::Char:
    return env->createString( NEW_USTR( QString( "%1" ).arg( *( static_cast<char *>( value ) ) ) ) );
  case QMetaType::UChar:
    return env->createString( NEW_USTR( QByteArray( 1, static_cast<uchar *>( value ) [ 0 ] ) ) );
  case QMetaType::Long:
    return env->createNumber( *( static_cast<long *>( value ) ) );
  case QMetaType::ULong:
    return env->createNumber( *( static_cast<ulong *>( value ) ) );
  default:
    return QSObject();
  };
}
...
QSObject convert_qt2qsa( QSEnv *env, void *value, const QSATypeInfo &ti, QObject *qobj )
{
  ...
  if ( ti.id < int( QVariant::UserType ) && ret_val.isValid() ) {
    ...
    QVariant * var = static_cast<QVariant *>( value );
    if ( var->isNull() )
      return env->createUndefined();
    return qsa_script_variant( ip, var->type(), const_cast<void *>( var->constData() ) );

  } else if ( ti.name == "QList<int>" ) {
    QList<int> *l = static_cast<QList<int> *>( value );
    QSArray array( env );
    for ( int i = 0; i < l->size(); ++i )
      array.put( NEW_USTRNUM( i ), env->createNumber( l->at( i ) ) );
    return array;

  } else if ( ti.name == "QObjectList" ) {
    QObjectList * l = static_cast<QObjectList *>( value );
    QSArray array( env );
    for ( int i = 0; i < l->size(); ++i )
      array.put( NEW_USTRNUM( i ), ip->wrap( l->at( i ) ) );
    return array;
    ...
    if ( star_count ) {
      ...
      if ( meta_object ) {
        return ip->wrap( *( static_cast<QObject **>( value ) ) );
    ...
    printf( "::convert_qt2qsa(), unhandled type id=%d, name='%s'\n",
            ti.id, static_cast<const char *>( ti.name ) );
  }
...
static void *qsa_default_value( QSASlotCaching *caching, const QSATypeInfo &ti, const QMetaObject *mo )
{
  ...
  void *const_data = const_cast<void *>( v->constData() );
