...
public:
    ...
    QMap<uint, QSObjectFactory*> objectsCache;
    QMap<uint, QList<QSWrapperFactory *> > wrappersCache;
    QMap<uint, QString> instanceDescriptors;
    QMap<uint, QObject*> staticDescriptors;
    QStringList classes;
};
                               ...
                               QuickDispatchObjectFactoryPrivate *d )
{
    ...
    QMap<uint, QString> descriptors = factory->instanceDescriptors();
    for ( QMap<uint, QString>::ConstIterator it = descriptors.begin();
            it != descriptors.end(); ++it ) {
        if ( !d->objectsCache.contains( it.key() ) ) {
            d->objectsCache[ it.key() ] = factory;
            d->instanceDescriptors[ it.key() ] = *it;
            d->classes << USTR( it.key() );
        } else {
            qWarning( "QuickDispatchObjectFactory::addObjectFactory()\n"
                      "  class '%s' is already registerd", USTR( it.key() ).toLatin1().constData() );
        }
    }

    QMap<uint, QObject*> sdesc = factory->staticDescriptors();
    for ( QMap<uint, QObject*>::ConstIterator sit = sdesc.begin();
            sit != sdesc.end(); ++sit ) {
        if ( !d->staticDescriptors.contains( sit.key() ) ) {
            d->staticDescriptors[ sit.key() ] = *sit;
            d->classes << USTR( sit.key() );
        } else {
            qWarning( "QuickDispatchObjectFactory::addObjectFactory()\n"
                      "  static class '%s'' is already registered", USTR( sit.key() ).toLatin1().constData() );
        }
    }
}
                                ...
                                QuickDispatchObjectFactoryPrivate *d )
{
    ...
    QList<uint> classes = factory->wrapperDescriptors().keys();
    for ( int i = 0; i < classes.size(); ++i ) {
        d->wrappersCache[ classes.at( i ) ].append( factory );
        d->classes << USTR( classes.at( i ) );
    }
}
...
bool QuickDispatchObjectFactory::createInterface( const QByteArray &className, void *ptr,
        QVector<QObject *> *result )
{
    const QList<QSWrapperFactory *> factories = d->wrappersCache.value( HASH_USTR( className ) );
    bool added = false;
...
bool QuickDispatchObjectFactory::createInstance( const QString &className,
        ...
        QVector<QObject *> *result )
{
    if ( !d->objectsCache.contains( HASH_USTR( className ) ) )
        return false;

    QSObjectFactory *factory = d->objectsCache[ HASH_USTR( className ) ];
    QObject *ctx = 0;
...
QMap<uint, QString> QuickDispatchObjectFactory::instanceDescriptors() const
{
    return d->instanceDescriptors;
}


QMap<uint, QObject*> QuickDispatchObjectFactory::staticDescriptors() const
{
