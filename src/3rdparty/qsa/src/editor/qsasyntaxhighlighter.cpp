    ...
    : Q3TextPreProcessor(), lastFormat( 0 ), lastFormatId( -1 )
{
    ...
    QString commentFamily = QLatin1String("times");
	...
	map[ QLatin1String(keywords[ i ]) ] = Keyword;
    }
...
static bool checkFunctionEnd( Q3TextParagraph *p, int index )
{
	...
	while ( true ) {
	    ...
	    text.prepend( c );
	    if ( text.startsWith( QLatin1String("function") )
                 || text.startsWith( QLatin1String("constructor") ) ||
		 text.startsWith( QLatin1String("class") ) /*|| text.startsWith( "if" ) || text.startsWith( "for" ) ||
...
void QSASyntaxHighlighter::process( Q3TextDocument *doc, Q3TextParagraph *string, int, bool invalidate )
{
    ...
    static QString alphabeth = QLatin1String("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ");
    static QString mathChars = QLatin1String("xXeE");
    static QString numbers = QLatin1String("0123456789");
	...
	if ( lastWasBackSlash ) {
	    ...
	    case ' ':
		input = InputSpace;
		if ( firstWord == QLatin1String("function") || firstWord == QLatin1String("constructor") || firstWord == QLatin1String("class")/* ||
		...
		if ( s == QLatin1String("private") || s == QLatin1String("protected") || s == QLatin1String("public") || s == QLatin1String("static") )
		    firstWord = "";
		if ( !questionMark && c == '?' )
		    questionMark = true;
		if ( c.isLetter() || c == '_' )
	...
	case StateStandard: {
		...
		if ( buffer == QLatin1String("true") || buffer == QLatin1String("false") ||
		     buffer == QLatin1String("NaN") || buffer == QLatin1String("Infinity") ||
		     buffer == QLatin1String("undefined") ) {
