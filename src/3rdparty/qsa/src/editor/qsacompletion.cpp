                      ...
                      bool super, bool withArgs, bool sigs )
{
  while (meta) {
    ...
    for (int i=0; i<nmethods; ++i) {
      ...
      if (m.access() == QMetaMethod::Private || m.access() == QMetaMethod::Protected)
        continue ;
      ...
      if ((m.methodType() == QMetaMethod::Slot && !sigs) ||
        ...
        if ( withArgs ) {
          ...
          for ( int j = 0; j < parameterTypes.count(); ++j ) {
            ...
            s += QLatin1String(" ");
            s += QLatin1String(parameterNames.at(j));
...
{
  QString str = s.mid( s.find( QLatin1String("(") ) + 1, s.find( QLatin1String(")") ) - 1 - s.find( QLatin1String("(") ) );
  str = str.simplifyWhiteSpace();
  QStringList lst = QStringList::split( ',', str );
  QStringList res;
  for ( QStringList::Iterator it = lst.begin(); it != lst.end(); ++it ) {
    QString arg( *it );
    arg = arg.replace( QRegExp( QLatin1String("const") ), QLatin1String("") );
    arg = arg.replace( QRegExp( QLatin1String("&") ), QLatin1String("") );
    arg = arg.replace( QRegExp( QLatin1String("*") ), QLatin1String("") );
    ...
    QSObject &obj )
{
  ...
  for ( int j = 0; j < num; ++j ) {
    ...
    if ( !mp.isScriptable())
      continue; 
    if ( propMap.find( QLatin1String(mp.name()) ) != propMap.end() )
      continue;
    CompletionEntry c;
  ...
  for ( int i = 0; i < code.length(); ++i ) {
    ...
    if ( c == ';' || c == '{' || c == '}' ) {
      if ( state == LeftHandSide ) {
        ...
        rightHandBuffer = rightHandBuffer.replace( QRegExp( QLatin1String("\\s") ), QLatin1String("") );
        leftHandBuffer = leftHandBuffer.replace( QRegExp( QLatin1String("\\s") ), QLatin1String("") );
...
QString QSACompletion::functionCode() const
{
  ...
  int braceCount = -1;
  QString s;
  while ( p && ( pos = p->string()->toString().find( QLatin1String("function") ) ) == -1 ) {
    s = p->string()->toString();
    braceCount += s.count( '{' );
    braceCount -= s.count( '}' );
    p = p->prev();
...
{
  int i = 0;
  while ( QLatin1String(QSASyntaxHighlighter::keywords[ i ]) != QString() )
    addCompletionEntry( QLatin1String(QSASyntaxHighlighter::keywords[ i++ ]), 0, false );
}
...
bool QSACompletion::doObjectCompletion( const QString &obj )
{
  ...
  if( obj == object ) {
    QSObject stobj = env()->globalObject().get( NEW_USTR( obj ) );
...
#endif
    if( o.qsobj.objectType()->name() == QLatin1String("FactoryObject") ) {
    ...
    QString &postfix )
{
  ...
  // some hardcoded ones
  if ( f == QLatin1String("startTimer") ) {
    QStringList args;
    args << QLatin1String("interval : Number");
    args << QLatin1String("callback : Function");
    postfix = QLatin1String(" : Number");
    l << args;
    return l;
  } else if ( f == QLatin1String("killTimer") ) {
    QStringList args;
    args << QLatin1String("id : Number");
    l << args;
    return l;
  } else if ( f == QLatin1String("connect") ) {
    QStringList args;
    args << QLatin1String("sender : QObject");
    args << QLatin1String("signal : String");
    args << QLatin1String("receiver : QObject");
    args << QLatin1String("slot : String");
    l << args;
    args.clear();
    args << QLatin1String("sender : QObject");
    args << QLatin1String("signal : String");
    args << QLatin1String("receiver : Function");
    l << args;
    return l;
  }
  ...
  if ( pnt == -1 ) {
    if ( !thisObject )
      return l;
    object.prepend( QLatin1String("this.") );
    pnt = object.findRev( '.' );
  }
  ...
  if( obj.qsobj.isValid()
      && obj.qsobj.objectType()->name() == QLatin1String("FactoryObject") ) {
  ...
  if ( funcs.isEmpty() ) {
    if ( f[ 0 ] == '\"' ) {
      ...
      if ( line.findRev( QLatin1String("connect("), cursor->index() ) != -1 ) {
        int i = line.findRev( ',' );
        static QString legalChars = QLatin1String("abcdefghijklmnopqrstuvwxyzABSCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_.");
        int end;
        for ( end = i; end >= 0; --end ) {
        ...
        func = f;
        func.remove( 0, 1 );
        func = func.left( func.find( QLatin1String("(") ) );
  ...
  for ( QStringList::Iterator fit = funcs.begin(); fit != funcs.end(); ++fit ) {
    ...
    for ( QStringList::Iterator it2 = argList.begin(); it2 != argList.end(); ++it2 ) {
      ...
      if ( l.count() > 1 )
        type = l[ 1 ] + QLatin1String(" : ") + type;
      res << type;
    }

    if ( !returnType.isEmpty() )
      postfix = QLatin1String(" : ") + returnType;
...
{
  QSObject qstype = env()->globalObject().get( NEW_USTR( type.mid( 1 ) ) );
  ...
  case QVariant::DateTime:
    return env()->globalObject().get( NEW_USTR( QLatin1String("Date") ) );
  case QVariant::StringList:
  case QVariant::List:
  case QVariant::Map:
    return env()->globalObject().get( NEW_USTR( QLatin1String("Array") ) );
  case QVariant::CString:
    return env()->globalObject().get( NEW_USTR( QLatin1String("String") ) );
  case QVariant::Int:
  case QVariant::UInt:
  case QVariant::Double:
    return env()->globalObject().get( NEW_USTR( QLatin1String("Number") ) );
  case QVariant::Bool:
    return env()->globalObject().get( NEW_USTR( QLatin1String("Boolean") ) );
  default:
    break;
  }
...
QString QSACompletion::cppClassForScript( const QString &className ) const
{
  ...
  QMap<uint,QString> names = ip->dispatchObjectFactory()->instanceDescriptors();
  return names.value( NEW_USTR( className ) );
}
...
QSObject QSACompletion::queryQSObject( QSObject &ctx, const QString &property ) const
{
  ...
  QSObject ctxValue = ctx.get( NEW_USTR( s ) );
  ...
  if ( s.find( '[' ) != -1 )
    return QSArray( env() );
  else if ( s == QLatin1String("false") || s == QLatin1String("true") )
    return env()->createBoolean( false );
  else if ( s[0] == '\'' || s[0] == '\"' )
    return env()->createString( NEW_USTR( QLatin1String("") ) );
  return ctxValue;
}
...
QSObject QSACompletion::queryQSObject( const QMetaObject *meta, const QString &property, bool /*includeSuperClass*/ ) const
{
  ...
  if (propertyIndex >= 0) {
    ...
    QSObject o = vTypeToQSType( QLatin1String(mp.typeName()) );
...
QVector<QObject *> QSACompletion::queryQObject( const QVector<QObject *> &objects, const QString &property ) const
{
  ...
  if ( obj.isA( interpreter()->wrapperClass() ) ) {
    QSMember m;
    if ( interpreter()->wrapperClass()->member( &obj, NEW_USTR( property ), &m ) ) {
      QSObject o = interpreter()->wrapperClass()->fetchValue( &obj, m );
      if ( o.isA( interpreter()->wrapperClass() ) )
        return *interfaceObjects( o );
    } else if ( interpreter()->applicationClass()->member( &obj, NEW_USTR( property ), &m ) ) {
