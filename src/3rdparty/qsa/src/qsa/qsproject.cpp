...
uint needsRerun :
  1;
uint scriptsModified :
  1;
uint bundleStorage :
  1; // tells saveInternal/loadInternal to store as binary bundle
};
                                  ...
                                  const char *qtscriptFunction )
{
  ...
  if ( receiver ) {
    ...
    function_ref = receiver_object.get( NEW_USTR( qtscriptFunction ) );
...
void QSProject::evaluate()
{
  ...
  for ( QList<QSScript *>::const_iterator it = d->scripts.begin(); it != d->scripts.end(); ++it ) {
    ip->execute( ( *it ) ->context(), ( *it ) ->code(), ( *it ) ->name() );
    if ( ip->hadError() ) {
...
void QSProject::initEventHandlers()
{
  ...
  for ( QList<QSSignalHandler>::Iterator it = d->signalHandlers.begin();
    ...
    if ( sigh.receiver ) {
      ...
      function_ref = receiver_object.get( NEW_USTR( sigh.function ) );
    } else {
...
void QSProject::objectDestroyed()
{
  ...
  if ( d->editors.removeAll( static_cast<QSEditor *>( const_cast<QObject *>( o ) ) ) )
    return ;
#endif

  if ( d->scripts.removeAll( static_cast<QSScript *>( const_cast<QObject *>( o ) ) ) ) {
    Q_EMIT projectChanged();
    return ;
  }
