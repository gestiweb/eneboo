...
QVariant QSInterpreter::evaluateByteCode( const QByteArray &code, QObject *context,
    const QString &scriptName )
{
  if ( context
       && context->isWidgetType()
  ...
  running = true;
  startInterpreter();
  QVariant v = d->interpreter->executeByteCode( context, code, scriptName );
  running = false;
  stopInterpreter();
  return v;
}

QVariant QSInterpreter::evaluateByteCode( QDataStream & in, QObject *context,
    const QString &scriptName )
{
  if ( context
       && context->isWidgetType()
  ...
  running = true;
  startInterpreter();
  QVariant v = d->interpreter->executeByteCode( context, in, scriptName );
  running = false;
  stopInterpreter();
  return v;
}
...
void QSInterpreter::parseError()
{
  ...
  sourceId = d->interpreter->debuggerEngine() ->sourceId();
#endif
...
QString QSInterpreter::aqScriptAndLineState() const
{
  return d->interpreter->nameOfSourceId( d->interpreter->lexer() ->sourceId() ) +
         QLatin1Char( ':' ) +
         QString::number( d->interpreter->lexer() ->runlineNo() );
}
...
void QSInterpreter::runtimeError( const QString &message,
                                  ...
                                  int lineNumber )
{
  Q_EMIT error( message, scriptName, lineNumber );

#ifdef QSDEBUGGER

  QObject *ctx = d->interpreter->objectOfSourceId( d->interpreter->debuggerEngine() ->sourceId() );
  Q_EMIT error( message, ctx, scriptName, lineNumber );
#endif
    ...
    const char *qtscriptFunction )
{
  ...
  QString func = QLatin1String( qtscriptFunction );
...
{
  throwError( QLatin1String( "Execution terminated" ) );
}
