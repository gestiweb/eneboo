...
static const char *patterns[] = { "13", "31" };

/* this is ordered in decades to simplify encoding */
static char alphabet[] = "0123456789" "ABCDEF";

/* stop sequence may be 231311313 (barcodemill.com) */
static const char *fillers[] = { "031311331", "331311313" };
...
static int add_one( char *ptr, int code ) {
	sprintf( ptr, "%s%s%s%s",
	         patterns[ code & 1 ],
	         patterns[ ( code >> 1 ) & 1 ],
	         patterns[ ( code >> 2 ) & 1 ], patterns[ ( code >> 3 ) & 1 ]
	       );
	return 0;
}
...
int Barcode_pls_encode( struct Barcode_Item *bc ) {
	...
	partial = ( char * ) malloc( ( strlen( text ) + 4 ) * 8 + 3 );
	checkptr = ( char * ) calloc( 1, strlen( text ) * 4 + 8 );
