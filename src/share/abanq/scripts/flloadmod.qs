...
function main() 
{
	...
	if ( nombreFichero ) {
		...
		if ( xmlModule.setContent( f ) ) {
			...
			if ( !nodeModule ) {
				MessageBox.critical( util.translate( "scripts", "Error en la carga del fichero xml .mod" ),
...
function evaluarDependencias( dependencias ) {
	...
	for ( var i = 0; i < dependencias.length; i++ ) {
		...
		if ( sys.isLoadedModule( dependencias[ i ] ) == false ) {
			res = MessageBox.warning( util.translate( "scripts", "Este módulo depende del módulo " )
			                          + dependencias[ i ]
			                          + util.translate( "scripts",
